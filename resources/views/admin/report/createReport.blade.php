...
<section class="row">
	...
	<div class="my-3 p-3 rounded">
		...
		<form method="post" action="{{ route('admin.storeReport') }}" enctype="multipart/form-data">
        @csrf
            {{-- Title --}}
            ...
            <div class="mb-3">
                <label for="konsumen_id" class="form-label">Nama Konsumen</label>
                <select class="form-select bg-gray border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500" id="konsumen_id" name="konsumen_id">
                    <option selected disabled>-- Pilih Nama Konsumen --</option>
                    @foreach($konsumen as $k)
                    <option value="{{ $k->id }}"
                            data-nohp="{{ $k->no_hp }}"
                            data-perumahan="{{ $k->perumahan }}"
                            data-sumber="{{ $k->sumber_informasi }}">
                        {{ $k->nama_konsumen }}
                    </option>
                    @endforeach
                </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="perumahan" name="perumahan" readonly>
        </div>

        <div class="mb-3">
            <label for="sumber_informasi" class="form-label">Sumber Informasi</label>
            <input type="text" class="form-control" id="sumber_informasi" name="sumber_informasi" readonly>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="follow_up" name="follow_up" value="FU1" readonly>
        </div>
